...
def menu():
    ...
    if sex in ['1']:
        ...
        os.system('xdg-open https://www.facebook.com/YUBRAJ BASNET;menu()
    elif sex in ['0']:
        sys.exit()
    else:
        menu()
...
def bd():
    ...
    with ThreadPool(max_workers=30) as sexy:
        ...
        print(f"{G1}[{A}={G1}]{G1}  TURN ON FLIGHT MOD linex()
        for love in user:💝
            ids = code + love
            ax = ids[:8]
            bx = ids[:7]
